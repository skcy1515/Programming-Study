...
# 디맨드 페이징
요구페이징은 프로세스가 실행되는 동안 필요한 페이지만 메모리에 올리고, 필요하지 않은 페이지는 디스크에 저장하여 메모리를 절약하는 방법이다. 이를 위해 페이지 테이블에 페이지의 위치 정보와 함께 각 페이지의 접근 여부를 표시하여 필요한 페이지만 메모리에 올리게 된다. 요구 페이징은 필요한 페이지만 적재하므로 프로세스 실행속도를 높이고, 메모리 사용이 효율적이다.

### 요구 페이징 작동 원리
![image](https://github.com/skcy1515/Programming-Study/assets/140364849/7a590e87-406b-41ba-98db-28379d15eeec)

![image](https://github.com/skcy1515/Programming-Study/assets/140364849/041279ec-085a-42dc-bc9a-55d2bc7a76fe)

1. 프로세스 실행 시점 : 프로세스가 실행되면 운영체제는 해당 프로세스의 가상 주소 공간을 만든다. 이 가상 주소 공간은 물리적 메모리 공간과는 별도의 공간으로 존재하며, 필요한 페이지만 물리적 메모리에 적재된다.
2. 페이지 부재(Page Fault) 발생 시 : 프로세스가 실행 중인데 필요한 페이지가 메모리에 없으면 페이지 부재(Page Fault)가 발생한다. 이 때, 운영체제는 해당 페이지를 디스크에서 메모리로 로드한다.
3. 페이지 교체 : 메모리 공간이 부족해져서 새로운 페이지를 로드해야 할 때, 운영체제는 메모리에 적재된 페이지 중에서 가장 오랫동안 사용되지 않은 페이지를 디스크로 옮기고, 새로운 페이지를 메모리에 적재한다. 이를 페이지 교체(Page Replacement)라고 한다.
4. 페이지 교체 알고리즘 : 페이지 교체 알고리즘은 페이지 교체 시 어떤 페이지를 교체할지 결정하는 알고리즘이다. 대표적인 페이지 교체 알고리즘으로는 LRU(Least Recently Used)알고리즘이 있다. 
5. 페이지 적재 시점 : 페이지가 처음으로 적재될 때, 페이지 테이블에 해당 페이지가 메모리에 적재되었다는 정보를 저장한다. 이후에는 해당 페이지를 참조할 때마다 페이지 테이블을 검색하여 메모리에 적재되어 있는지 확인하고, 없으면 페이지 테이블을 검색하여 메모리에 적재되어 있는지 확인하고, 없으면 페이지 부재가 발생한다.
6. 페이지 디스크에 저장 : 요구페이징에서는 메모리에 적재되어 있지 않은 페이지는 디스크에 저장된다. 디스크에 저장되는 페이지는 스왑 파일(Swap File)이라는 특수한 파일에 저장된다.
 

### 페이지 폴트(Page Fault)
가상 메모리 관리 방식에서는 발생하여 현상으로, 프로세스가 실행되는 도중에 필요한 페이지가 물리 메모리에 존재하지 않아 디스크에서 해당 페이지를 가져와야 하는 경우를 말한다. 즉, 프로세스가 접근하려는 페이지가 물리적 메모리에 존재하지 않는 상태에서 발생하는 인터럽트입니다. 

페이지 폴트가 발생하면 운영체제는 해당 페이지를 물리 메모리에 적재하여 프로세스가 접근할 수 있도록 해야 한다. 이때, 메모리에 빈공간이 있다면 페이지 폴트를 처리하기 위해 디스크에서 해당 페이지를 물리 메모리에 적재하여 프로세스가 접근할 수 있도록 해야 한다. 이때, 메모리에 빈 공간이 있다면 페이지 폴트를 처리하기 위해 디스크에서 해당 페이지를 읽어와 물리 메모리에 적재한다. 그러나 빈 공간이 없는 경우, 운영체제는 페이지 교체(Page Replacement) 알고리즘을 사용하여 더 이상 사용되지 않는 페이지를 디스크로 내보내고, 새로운 페이지를 메모리에 적재한다.

페이지 폴트가 발생하는 빈도는 페이지 교체 알고리즘과 메모리의 크기 프로세스의 특성 등에 따라 달라진다. 페이지 폴트가 많이 발생하면 시스템의 성능이 저화되므로, 페이지 교체 알고리즘을 최적화하여 페이지 폴트를 최소화하는 것이 중요하다.

# 쓰레싱과 워킹셋
다중 프로그래밍 정도(degree of multiprogramming)가 계속 늘어나다 보면 어느 순간부터 페이지 부재(page fault)가 과도하게 늘어나 실제 CPU 사용 시간보다 페이지를 교체(page replacement) 하는 시간이 더 많이 지게 되는데, 이러한 현상을 스레싱이라고 한다. 다중 프로그래밍 정도가 늘어난다는 의미는 병행 실행되는 프로세스의 수가 늘어난다는 의미

### 스레싱의 원인
1. 현재 고정된 페이지 프레임 수가 충분하지 않음
2. page fault가 자주 발생, CPU 사용률이 떨어짐
3. OS 입장에서는 CPU 사용률이 떨어지기 때문에 다중 프로그래밍 정도를 높임(새로운 프로세스 추가), page fault 더욱 자주 발생, CPU 사용률이 더욱 떨어짐
4. 다중 프로그래밍 정도를 더 높임 ... -> 무한 반복

### 스레싱 예방 - 워킹 셋(Working Set)
- OS는 다중 프로그래밍 수준과 CPU 사용률을 비교하여 스레싱 탐색 -> 우선순위 알고리즘이나 지역 교환 알고리즘 사용
- 이러한 방법으로 스레싱을 처리하면 탐색에 많은 리소스를 소모함 -> 스레싱 현상을 예방하는 것이 최선(워킹 셋 사용)
- 워킹 셋(Working Set): 지역성을 기반으로 가장 많이 사용하는 페이지를 미리 저장해둔 것
- 워킹 셋에 있는 page들은 page frame에 고정시켜 두는 방법으로 다양한 응용이 가능, page fault 감소
